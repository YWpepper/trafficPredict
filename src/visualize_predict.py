...
def show_pred(test_loader, all_y_true, all_predict_values):
    ...
    plt.legend()
    plt.savefig("../result/the first day pred flow in node {}.png".format(str(node_id)), dpi=400)
    plt.show()
    ...
    plt.legend()
    plt.savefig("../result/the first two weeks pred flow in node {}.png".format(str(node_id)), dpi=400)
    plt.show()
    ...
    mape = MAPE(test_loader.dataset.recover_data(test_loader.dataset.flow_norm[0], test_loader.dataset.flow_norm[1],
                ...
                test_loader.dataset.recover_data(test_loader.dataset.flow_norm[0], test_loader.dataset.flow_norm[1],
                                                 all_predict_values[0]))
    print("CSTGN基于原始值的精度指标  mae: {:02.4f}, rmse: {:02.4f}, mape: {:02.4f}".format(mae, rmse, mape))
